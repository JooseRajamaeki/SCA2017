...
static const bool optimizeFmaxInVelocityMode=false;
...
public:
	...
	virtual void applyControl(const float *control)
	{
		...
		for (size_t motorIdx=0; motorIdx<joints.size(); motorIdx++)
		{
			if (joints[motorIdx]->nMotorDof==3)
			{
				...
				if (useTorque)
				{
					odeJointSetAMotorParam(motor,dParamFMax1,fmax[0]);
					odeJointSetAMotorParam(motor,dParamFMax2,fmax[1]);
					odeJointSetAMotorParam(motor,dParamFMax3,fmax[2]);
					odeJointAddAMotorTorques(motor,c[0],c[1],c[2]);
				}
				else
				{
			...
			else if (joints[motorIdx]->nMotorDof==1)
			{
				...
				if (useTorque)
				{
					odeJointSetHingeParam(motor,dParamFMax,fmax[0]);
					odeJointAddHingeTorque(motor,c[0]);
				}
				else
				{
		...
		//adjust fmaxes if used
		if (optimizeFmaxInVelocityMode && !useTorque)
		{
			for (size_t motorIdx=0; motorIdx<joints.size(); motorIdx++)
			{
				if (joints[motorIdx]->nMotorDof>0)
				{
					float f=controlFmaxScale*control[varIdx++];
					if (joints[motorIdx]->nMotorDof==3)
					{
						odeJointSetAMotorParam(joints[motorIdx]->motor,dParamFMax1,f);
						odeJointSetAMotorParam(joints[motorIdx]->motor,dParamFMax2,f);
						odeJointSetAMotorParam(joints[motorIdx]->motor,dParamFMax3,f);
					}
					else if (joints[motorIdx]->nMotorDof==1)
					{
						odeJointSetHingeParam(joints[motorIdx]->joint,dParamFMax,f);
					}
				}
			}
		}
	}
	void getCurrentMotorAngles(float *out_angles) const
	{
		int varIdx=0;
	...
	Joint *createHinge(int body1, int body2, const Vector3f &anchor, const Vector3f &axis,float minAngle, float maxAngle)
	{
		...
		if (useTorque)
			odeJointSetHingeParam(joint,dParamFMax,torqueControlStabilizingFmax); 
		else
	...
	Joint *createMotoredBallJoint(int body1, int body2, const Vector3f &anchor, const Vector3f &axis0, const Vector3f &axis2, const Vector3f &angleMin, const Vector3f &angleMax)
	{
		...
		if (useTorque)
		{
			odeJointSetAMotorParam(joint,dParamFMax1,torqueControlStabilizingFmax);
			odeJointSetAMotorParam(joint,dParamFMax2,torqueControlStabilizingFmax);
			odeJointSetAMotorParam(joint,dParamFMax3,torqueControlStabilizingFmax);
		}
		else
		{
