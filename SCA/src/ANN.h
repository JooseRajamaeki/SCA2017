...
public:
	...
	std::vector<std::unique_ptr<CandidateSolution>> pso_population_;
	...
	void build_input_connected_network(const std::vector<unsigned>& layer_widths);
	void build_structured_network(const unsigned input_dim, const unsigned output_dim, const unsigned layer_width_factor, std::vector<std::vector<unsigned>> input_groupings, std::vector<std::vector<unsigned>> output_groupings);
	void build_sigmoid_network(const std::vector<unsigned>& layer_widths);
	void build_residual_network(const std::vector<unsigned>& layer_widths);
	...
	void train_particle_swarm_optimization(const float** in, const float** out, unsigned data_points,int population);
	...
	void increase_capacity(int operation_idx, int dimension);
	void store_parameters_to_pso_population();

	std::pair<int,int> find_hidden_neuron_with_biggest_variation(const float** in, unsigned data_points);
	std::vector<std::pair<int, int>> find_hidden_neurons_with_biggest_variation(const float** in, unsigned data_points);
	std::pair<int, int> find_hidden_neuron_with_greatest_blame(const float** in, const float** out, unsigned data_points);
	std::vector<std::pair<int, int>> find_hidden_neurons_with_greatest_blame(const float** in, const float** out, unsigned data_points);
